...
const handleUpdatePassword = async () => {
  if (passwordForm.value.new_password !== passwordForm.value.confirm_password) {
    showNotification('รหัสผ่านใหม่ไม่ตรงกัน กรุณาตรวจสอบอีกครั้ง', 'error')
    return
  }
...
// ดึงตัวอักษรตัวแรกของ Email มาทำเป็นรูปโปรไฟล์ชั่วคราว
...
<template>
  <div class="min-h-screen bg-[#050505] pt-28 pb-20 px-6 md:px-12 font-sans animate__animated animate__fadeIn">
    ...
    <div class="max-w-5xl mx-auto">
      <h1 class="text-3xl md:text-4xl font-bold text-white mb-8 border-l-4 border-[#00e054] pl-4">ตั้งค่าโปรไฟล์ส่วนตัว</h1>

      <div v-if="isLoading" class="text-center py-20 text-[#00e054]"><Loader2 class="w-8 h-8 animate-spin mx-auto mb-2" /> กำลังโหลดข้อมูล...</div>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div class="lg:col-span-1">
          <div class="glass-card p-6 md:p-8 rounded-2xl border border-white/5 flex flex-col items-center text-center bg-[#0a0a0a]">
            ...
            <h2 class="text-xl font-bold text-white mb-1 truncate w-full px-2">{{ profileForm.display_name || 'ไม่ได้ตั้งชื่อ' }}</h2>
            ...
            <div class="w-full pt-6 border-t border-white/5 text-left space-y-3">
              ...
              <div class="flex items-center gap-3 text-sm text-gray-400">
                <Calendar class="w-4 h-4 text-gray-500" /> เข้าร่วมเมื่อ: {{ joinDate }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6 lg:space-y-8">
          ...
          <div class="glass-card p-6 md:p-8 rounded-2xl border border-white/5 bg-[#0a0a0a]">
            <h2 class="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <User class="w-5 h-5 text-[#00e054]" /> แก้ไขข้อมูลทั่วไป
            </h2>
            <form @submit.prevent="handleUpdateProfile" class="space-y-5">
              <div class="grid grid-cols-1 gap-5">
                <div class="space-y-2">
                  <label class="text-sm font-medium text-gray-300">ชื่อแสดงผล (Display Name)</label>
                  <input v-model="profileForm.display_name" type="text" placeholder="ตั้งชื่อเล่นของคุณ" class="w-full px-4 py-3 bg-black/40 border border-gray-700 rounded-xl text-white focus:border-[#00e054] outline-none transition-colors">
                </div>
                <div class="space-y-2">
                  <label class="text-sm font-medium text-gray-300">ลิงก์รูปโปรไฟล์ (Avatar URL)</label>
                  <div class="relative">
                  ...
                  <p class="text-xs text-gray-500 mt-1">ใส่ลิงก์รูปภาพนามสกุล .jpg, .png</p>
                </div>
              </div>
              <div class="pt-2">
                <button type="submit" :disabled="isSavingProfile" class="bg-gray-800 hover:bg-[#00e054]/20 text-gray-300 hover:text-[#00e054] border border-gray-700 hover:border-[#00e054]/50 font-semibold px-6 py-2.5 rounded-xl transition-all flex items-center gap-2 disabled:opacity-50 text-sm">
                  ...
                  <Save v-else class="w-4 h-4" /> บันทึกข้อมูล
                </button>
              </div>
            </form>
          </div>
          ...
          <div class="glass-card p-6 md:p-8 rounded-2xl border border-white/5 bg-[#0a0a0a]">
            <h2 class="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <Lock class="w-5 h-5 text-yellow-500" /> เปลี่ยนรหัสผ่าน
            </h2>
            <form @submit.prevent="handleUpdatePassword" class="space-y-5">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="space-y-2">
                  <label class="text-sm font-medium text-gray-300">รหัสผ่านใหม่</label>
                  <input v-model="passwordForm.new_password" type="password" required placeholder="อย่างน้อย 6 ตัวอักษร" minlength="6" class="w-full px-4 py-3 bg-black/40 border border-gray-700 rounded-xl text-white focus:border-yellow-500 outline-none transition-colors">
                </div>
                <div class="space-y-2">
                  <label class="text-sm font-medium text-gray-300">ยืนยันรหัสผ่านใหม่</label>
                  <input v-model="passwordForm.confirm_password" type="password" required placeholder="กรอกรหัสผ่านอีกครั้ง" minlength="6" class="w-full px-4 py-3 bg-black/40 border border-gray-700 rounded-xl text-white focus:border-yellow-500 outline-none transition-colors">
                </div>
              </div>
              <div class="pt-2">
                <button type="submit" :disabled="isSavingPassword" class="bg-gray-800 hover:bg-yellow-500/20 text-gray-300 hover:text-yellow-500 border border-gray-700 hover:border-yellow-500/50 font-semibold px-6 py-2.5 rounded-xl transition-all flex items-center gap-2 disabled:opacity-50 text-sm">
                  ...
                  <Lock v-else class="w-4 h-4" /> อัปเดตรหัสผ่าน
                </button>
              </div>
            </form>
          </div>
