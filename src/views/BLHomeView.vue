...
const toggleBookmark = async (series) => {
  if (!currentUser.value) return alert('กรุณาเข้าสู่ระบบก่อนเพื่อเพิ่มในรายการของฉัน')
...
<template>
  <div>
    ...
    <div v-else-if="seriesList.length === 0" class="min-h-screen flex items-center justify-center text-gray-500">ยังไม่มีข้อมูลซีรีส์ BL ในระบบ</div>
    <div v-else>
          ...
          <transition name="fade-slide" mode="out-in">
            <div v-if="heroSeries" :key="heroSeries.id" class="max-w-2xl">
              ...
              <div class="flex items-center gap-2 mb-4 flex-wrap">
                <span class="text-[11px] font-bold text-white bg-[#00e054] px-2 py-0.5 rounded-sm uppercase">BL Series</span>
      ...
      <section class="relative z-10 px-6 md:px-12 pb-20 pt-10">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <h2 class="text-2xl font-bold text-white border-l-4 border-[#00e054] pl-3">ยอดนิยม (BL)</h2>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate__animated animate__fadeIn animate__faster">
      ...
      <div class="relative w-full max-w-4xl glass-card rounded-2xl overflow-hidden shadow-2xl border border-white/10 flex flex-col max-h-[90vh] bg-[#0d0d0d]">
        <button @click="closeModal" class="absolute top-4 right-4 z-10 w-8 h-8 flex items-center justify-center bg-black/50 hover:bg-blue-600 rounded-full text-white transition-colors backdrop-blur-sm"><X class="w-5 h-5" /></button>
