...
// แก้บั๊กจอดำ: สลับไปใช้ HQ อัตโนมัติถ้า Maxres ไม่มี
...
<template>
  <div>
    ...
    <div v-else>
        ...
        <div class="w-full px-6 md:px-16 relative z-10 pt-20">
          <transition name="fade-slide" mode="out-in">
            <div v-if="heroSeries" :key="heroSeries.id" class="max-w-2xl">
              ...
              <!-- ปุ่ม Play และ Bookmark สไตล์ iQIYI -->
      ...
      <!-- หมวดหมู่: ซีรีส์ทั้งหมด -->
      <section class="relative z-10 px-6 md:px-12 pb-20 pt-8">
        <h2 class="text-2xl font-bold text-white mb-6">ยอดนิยม (GL)</h2>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
          ...
          >
            ...
            <div class="aspect-video relative bg-gray-900 rounded-lg overflow-hidden">
              ...
              />
              <!-- ป้ายสีเขียวมุมขวาบน (สไตล์ iQIYI) -->
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate__animated animate__fadeIn animate__faster">
      ...
      <div class="relative w-full max-w-4xl glass-card rounded-2xl overflow-hidden shadow-2xl border border-white/10 flex flex-col max-h-[90vh] bg-[#0d0d0d]">
        <button @click="closeModal" class="absolute top-4 right-4 z-10 w-8 h-8 flex items-center justify-center bg-black/50 hover:bg-pink-600 rounded-full text-white transition-colors backdrop-blur-sm">
          <X class="w-5 h-5" />
        </button>
...
<style scoped>
/* Transition สำหรับ Fade รูปพื้นหลัง */
.fade-enter-active,
.fade-leave-active {
...
/* Transition สำหรับเลื่อนข้อความ */
...
.custom-scrollbar::-webkit-scrollbar-thumb {
  background: rgba(0, 224, 84, 0.5); /* สีเขียว iQIYI */
  border-radius: 10px;
}
</style>